...
void ParseTerrainData(TerrainData& terrain_data){
  ...
  Eigen::VectorXd x_scaled = x_shift*0.05;
  Eigen::VectorXd y_scaled = y_shift*0.05;
  Eigen::VectorXd z_scaled = z*0.5;
...
std::shared_ptr<TerrainFromData> MakeTerrainFromData() {
  TerrainData terrain_data;
  ParseTerrainData(terrain_data);
  return std::make_shared<TerrainFromData>(terrain_data);
}
